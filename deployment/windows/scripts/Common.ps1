...
function Ensure-Directories {
    foreach ($path in @($script:RuntimeDir, $script:BackupsDir, (Join-Path $script:RuntimeDir "surreal_data"), (Join-Path $script:RuntimeDir "notebook_data"))) {
...
function Ensure-EnvFile {
    Ensure-Directories
    if (-not (Test-Path $script:EnvPath)) { Copy-Item $script:EnvTemplatePath $script:EnvPath -Force }
...
function Assert-RequiredFile([string]$Path, [string]$FriendlyName) {
    if (-not (Test-Path $Path)) { throw "$FriendlyName no existe: $Path" }
}
function Assert-Docker {
    if (-not (Get-Command docker -ErrorAction SilentlyContinue)) { throw "Docker Desktop no está instalado o no está en PATH." }
    try { docker version | Out-Null } catch { throw "Docker Desktop parece no estar iniciado. Ábralo y vuelva a intentarlo." }
}
function Assert-Git {
...
function Invoke-Compose([string[]]$Args) {
    ...
    try { & docker compose --env-file $script:EnvPath -f $script:ComposePath @Args } finally { Pop-Location }
}
function Get-ListeningState([int]$Port) {
    try { return ($null -ne (Get-NetTCPConnection -State Listen -LocalPort $Port -ErrorAction Stop)) } catch { return $false }
}
