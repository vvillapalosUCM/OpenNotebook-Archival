...
try {
    ...
    if ([string]::IsNullOrWhiteSpace([string]$settings["OPEN_NOTEBOOK_PASSWORD"])) {
        do {
            ...
            if ($pwd.Length -lt 10) { Write-Warn "La contraseña debe tener al menos 10 caracteres." }
    ...
    foreach ($entry in @(
        ...
        if (Get-ListeningState $entry.Port) { Write-Warn ("El puerto {0} ({1}) parece estar ocupado." -f $entry.Port, $entry.Name) }
        else { Write-Ok ("El puerto {0} ({1}) está libre." -f $entry.Port, $entry.Name) }
    }
    ...
    if ($ollamaTest.Success) { Write-Ok ("Ollama responde correctamente en {0}" -f $settings["HOST_OLLAMA_URL"]) }
    else {
